...
There's no need for field-specific error messages either: decoders already report which
field failed.
...
single decode_. They should typically only _pick_ an existing decoder, not construct a new
one on every call.
...
const aspectRatio = numberPair(':'); // "16:9"
```
