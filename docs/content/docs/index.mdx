...
| ----------------------- | ---------------- | -------------------- |
| [`.verify()`](/.verify) | `T`              | Throws an error      |
| [`.value()`](/.value)   | `T \| undefined` | Returns `undefined`  |
| [`.decode()`](/.decode) | `Result<T>`      | Returns error result |

Most of the time, [`.verify()`](/.verify) is what you want.
...
By default, [`.verify()`](/.verify) uses `formatInline`, which echoes back the input with
errors inlined:
