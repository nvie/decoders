...
`string`, `number`, and `isoDate` are all individual decoders that get combined with
...
By default, [`.verify()`](/docs/api/Decoder#decoder.verify) uses `formatInline`, which
echoes back the input with errors inlined:
