...
[`string`](/docs/api/strings#string) decoder _accepts_ strings, and also _returns_ (those
same) strings.
...
| :------------------------------------- | :--------- | :------------- | :----------------- |
| [`string`](/docs/api/strings#string)   | strings    | strings        | `Decoder<string>`  |
| [`email`](/docs/api/strings#email)     | strings    | strings        | `Decoder<string>`  |
| [`number`](/docs/api/numbers#number)   | numbers    | numbers        | `Decoder<number>`  |
| [`integer`](/docs/api/numbers#integer) | numbers    | numbers        | `Decoder<number>`  |
| [`iso8601`](/docs/api/dates#iso8601)   | strings    | Date instances | `Decoder<Date>`    |
| [`url`](/docs/api/strings#url)         | strings    | URL instances  | `Decoder<URL>`     |
| [`truthy`](/docs/api/booleans#truthy)  | anything!  | booleans       | `Decoder<boolean>` |
...
built-in [`sized()`](/docs/api/utilities#sized) helper:
...
The [`sized()`](/docs/api/utilities#sized) helper also supports `min`, or an exact `size`.
...
To define the acceptance criteria, we'll use a [`regex()`](/docs/api/strings#regex)
decoder. This will enforce that only alphabetical chars are used, and that there are
exactly 5 of them (it's important that the regex pattern is anchored using `^` and `$` for
...
Wait, why not use [`nullable(iso8601)`](/docs/api/optional#nullable) here?! The reason is
the third event in the example. Because the field can legally be missing, we'll have to
explicitly accept both `undefined` and `null` inputs. That's what the
[`nullish()`](/docs/api/optional#nullish) decoder does! Its second argument is a
...
possible, and instead wrap them in [`nullable()`](/docs/api/optional#nullable)s where you
use them in your call sites - typically in those big
[`object()`](/docs/api/objects#object) decoders.
...
thus a more reusable, building block. It's cheap to wrap it in a
[`nullable`](/docs/api/optional#nullable) where you put it all together.
