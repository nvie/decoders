...
[`string`](/string) decoder _accepts_ strings, and also _returns_ (those same) strings.

This isn't always automatically the case, though. Some random examples:

| This decoder...       | ...accepts | ...but returns | ...so its type is  |
| :-------------------- | :--------- | :------------- | :----------------- |
| [`string`](/string)   | strings    | strings        | `Decoder<string>`  |
| [`email`](/email)     | strings    | strings        | `Decoder<string>`  |
| [`number`](/number)   | numbers    | numbers        | `Decoder<number>`  |
| [`integer`](/integer) | numbers    | numbers        | `Decoder<number>`  |
| [`isoDate`](/isoDate) | strings    | Date instances | `Decoder<Date>`    |
| [`url`](/url)         | strings    | URL instances  | `Decoder<URL>`     |
| [`truthy`](/truthy)   | anything!  | booleans       | `Decoder<boolean>` |
...
The [`sized()`](/sized) helper also supports `min`, or an exact `size`.
...
To define the acceptance criteria, we'll use a [`regex()`](/regex) decoder. This will
enforce that only alphabetical chars are used, and that there are exactly 5 of them (it's
...
Wait, why not use [`nullable(isoDate)`](/nullable) here?! The reason is the third event in
the example. Because the field can legally be missing, we'll have to explicitly accept
both `undefined` and `null` inputs. That's what the [`nullish()`](/nullish) decoder does!
Its second argument is a convenience default value that nullish values will get normalized
to.
...
possible, and instead wrap them in [`nullable()`](/nullable)s where you use them in your
call sites - typically in those big [`object()`](/object) decoders.
...
thus a more reusable, building block. It's cheap to wrap it in a [`nullable`](/nullable)
