...
description:
  Learn how to define custom decoders using .refine(), .transform(), and .pipe().
---
...
- Optionally, to feed that result into another decoder, use [`.pipe()`][]
...
<DecoderPlayground
  ...
  `}
  decoder="truncated(23)"
  examples={["'hello'", "'long strings will truncate!'", '123']}
/>
...
Suppose you want to build your own [Wordle](https://www.nytimes.com/games/wordle/) clone.
At the boundary of your program, you'll want to enforce that these are 5-letter words with
...
want to use that on other string decoders as well. Do you just stick [`.transform()`][]
after those?
...
```ts
function lower(decoder: Decoder<string>): Decoder<string> {
  return decoder.transform((s) => s.toLowerCase());
}

function upper(decoder: Decoder<string>): Decoder<string> {
  return decoder.transform((s) => s.toUpperCase());
}
```

Then you can reuse them anywhere in your decoders.
...
Now that you know how to build your own decoders, read
[Idiomatic decoders](/idiomatic-decoders) for a list of common mistakes to avoid, like
naming decoders after fields, or baking [`nullable()`][] into reusable decoders.
