...
<DecoderSig
  name="unknown"
  aliases={[{ name: 'anything', deprecated: false }]}
  type="unknown"
  source="basics.ts#L130-L137"
/>
...
<DecoderSig name="always" params="value: T" type="T" source="basics.ts#L99-L114" />
<DecoderSig name="always" params="value: () => T" type="T" />
...
<DecoderPlayground
  decoder="always(() => Math.random().toFixed(2))"
  examples={["'I will be ignored'", 123]}
/>
...
<DecoderSig
  name="never"
  aliases={[{ name: 'fail', deprecated: false }]}
  type="never"
  source="basics.ts#L116-L123"
/>
...
```ts
type SizeOptions =
  { size: number } | { min: number; max?: number } | { min?: number; max: number };
```

For strings, error messages refer to "chars". For arrays and sets, error messages refer to
"items".

For arrays, prefer [`array(decoder, options)`](/docs/api/arrays#limiting-array-sizes): it
checks the length before decoding any items.
...
<DecoderSig
  ...
  type="T"
  source="core/Decoder.ts#L373-L390"
/>
...
const decoder = prep(
  ...
  // natural. This will effectively also allow numeric strings
