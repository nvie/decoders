# Plans

Plans for larger changes live in `.claude/plans/<name>.md`. At the start of a
session, check if there are any existing plans. When asked to make a plan, write
it there. Use `- [x]`/`- [ ]` checkboxes to track progress, and update them as
you go.

# Commands

- pnpm run test: run the unit tests
- pnpm run test:typescript: run typecheck
- pnpm run test:types: run type-level tests
- pnpm run lint: reformat all code with linters
- pnpm run lint:package: check that the package is correct
